...
def render_results(result_df, trade_log, stats, rs, rw, mr, dd, feature_df=None,
    ...
    # --- Stress Test ---
    if feature_df is not None and generate_signal_fn is not None:
        st.markdown("### Stress Test")

        # --- Monte Carlo Noise Injection ---
        st.markdown(
            "#### Is the strategy robust to noise, or is performance fragile?"
        )
        mc_key = f"mc_result_{ticker_key}"

        # Auto-run MC with defaults on first load
        ...
        # --- Trade Bootstrap ---
        st.markdown(
            "#### Would a different trade ordering have changed the drawdown?"
        )
        ...
        st.markdown(
            "#### Does the strategy hold up in turbulent markets, or only work in calm ones?"
        )
