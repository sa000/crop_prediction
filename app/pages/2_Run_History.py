...
from app.style import (
    ...
    ACCENT, ACCENT_DIM, BG_CARD, BORDER, BORDER_SUBTLE,
...
from etl.db import (
    ...
    update_backtest_run_notes, delete_backtest_run,
)
...
filter_run_type = st.sidebar.selectbox(
    "Run Type", ["All", "manual", "paper"],
)
...
runs = list_backtest_runs(
    ...
    run_type=filter_run_type if filter_run_type != "All" else None,
...
for rank, run in enumerate(runs, 1):
    ...
    if run["run_type"] == "paper":
        type_badge = (
            ...
            f'font-size: 0.7rem; font-weight: 600; padding: 0.1rem 0.4rem; '
            f'border-radius: 4px; text-transform: uppercase; margin-left: 0.5rem;">'
            f'Paper</span>'
        )
    else:
        type_badge = (
            f'<span style="background: {GREEN}22; color: {GREEN}; '
            f'font-size: 0.7rem; font-weight: 600; padding: 0.1rem 0.4rem; '
            f'border-radius: 4px; text-transform: uppercase; margin-left: 0.5rem;">'
            f'Manual</span>'
        )
    ...
    st.markdown(
        ...
        f' &nbsp;&bull;&nbsp; {run["run_by"]}'
        f' &nbsp;&bull;&nbsp; {run["created_at"][:10]}</div>'
        f'</div>',
        unsafe_allow_html=True,
    )

    # Action buttons
    col_view, col_star, col_notes, col_del = st.columns([1, 1, 2, 1])
    ...
    with col_del:
        if st.button("Delete", key=f"del_{run_id}", width="stretch"):
        ...
        if st.session_state.get(f"confirm_del_{run_id}"):
            ...
            c1, c2 = st.columns(2)
            with c1:
                if st.button("Yes", key=f"yes_del_{run_id}", width="stretch"):
                    delete_backtest_run(conn, run_id)
                    st.session_state.pop(f"confirm_del_{run_id}", None)
                    st.rerun()
            with c2:
                if st.button("No", key=f"no_del_{run_id}", width="stretch"):
                    st.session_state.pop(f"confirm_del_{run_id}", None)
                    st.rerun()
